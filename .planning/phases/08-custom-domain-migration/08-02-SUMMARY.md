...
- **Full chain:** `npm run validate` → exit 0 (html-validate ✓, check-no-old-domain OK ✓, linkinator 18 local links 200s with the apex skip preventing self-crawl ✓, i18n-detect 23/23 ✓, 20 dictionaries × 170-key exact parity ✓). [corrected Phase 11: 19 JSON dictionaries — no en.json; EN is the markup baseline, so the "20" counted locales, not files]
